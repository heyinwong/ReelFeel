...
function MovieModalBack({
  ...
  return (
    <div className="absolute inset-0 bg-[#281B13] text-[#F3E2D4] p-6 overflow-y-auto max-h-[90vh] rounded-2xl shadow-lg [backface-visibility:hidden] [transform:rotateY(180deg)]">
      <h3 className="text-2xl font-black mb-6 text-center">
        Reflection for{" "}
        <span className="text-[#FC7023]">{movie.title}</span>
      </h3>

      {/* Thoughts */}
      <div className="px-6 mb-6">
        <textarea
          className="w-full h-36 bg-[#1d130e] border border-[#FC7023]/45 text-[#F3E2D4] rounded-xl px-4 py-3 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[#FC7023]"
          value={review}
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-6 px-6 mb-6">
        ...
        <div className="w-full flex justify-center sm:justify-end gap-3">
          <button
            className={`px-4 py-1.5 rounded-xl text-sm font-semibold shadow transition-transform duration-200 hover:scale-[1.02] ${
              localLiked
          ...
          >
            {localDisliked ? "Not for me" : "Not for me"}
          </button>
        </div>
      </div>
      ...
      <div className="mt-6 text-center border-t border-dotted border-[#FC7023]/30 pt-4">
        <p className="text-sm text-[#F3E2D4]/70">
          Ratings, moods, and notes become taste-memory signals.
        </p>
      </div>
      ...
      <div className="flex justify-center gap-4 mt-6">
        {onDelete && (
          <button
            className="px-6 py-2 rounded-xl bg-red-600 text-white hover:bg-red-700 transition hover:scale-[1.02]"
            onClick={() => {
              onDelete(movie);
              onClose();
            }}
        ...
        <button
          className="px-6 py-2 rounded-xl bg-[#FC7023] text-[#281B13] font-bold hover:bg-[#ff8c3a] transition hover:scale-[1.02] flex items-center gap-2"
          onClick={onSave}
          disabled={isSaving}
        >
        ...
        <button
          className="px-6 py-2 rounded-xl border border-[#F3E2D4]/50 text-[#F3E2D4] hover:bg-[#F3E2D4]/10 transition hover:scale-[1.02]"
          onClick={onFlipBack}
        >
          Back
        </button>
