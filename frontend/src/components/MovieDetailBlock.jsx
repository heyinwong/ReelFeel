...
function MovieDetailBlock({ movie, onAdd }) {
  ...
  return (
    <motion.div
      ...
      className="bg-gradient-to-br from-[#FAEFE0] to-[#F3E2D4] text-[#281B13] mt-14 rounded-3xl shadow-[0_8px_24px_rgba(252,112,35,0.15)] px-10 py-8 max-w-4xl mx-auto relative overflow-hidden border border-[#FC7023]/30 backdrop-blur-md"
    >
      {/* 背景点缀纹理 */}
      <div className="absolute inset-0 opacity-5 bg-[url('/film-texture.png')] bg-repeat pointer-events-none" />

      {/* 标题 */}
      <h3 className="text-3xl sm:text-4xl font-extrabold mb-2 tracking-wide text-center drop-shadow-sm">
        {movie.title}
      </h3>
      ...
      {/* 简介 */}
      <p className="text-sm sm:text-base leading-relaxed mb-6 px-2 text-center text-[#3b2c23]">
        {movie.description}
      </p>
      ...
      {movie.reason && (
        ...
        >
          ...
          <p className="text-sm italic text-[#4a3a30] leading-snug">
            {movie.reason}
          </p>
        </motion.div>
      )}
      ...
      <div className="flex justify-center flex-wrap gap-4 mt-4">
        <motion.button
          ...
          whileTap={{ scale: 0.97 }}
          className="bg-[#FC7023] hover:bg-orange-600 text-white px-5 py-2.5 rounded-full font-bold transition-all duration-200 shadow-md"
